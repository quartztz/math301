...
\aparte{Properties}{
  Some properties of polynomial rings over a field $F$:
  \begin{enumerate}
    \item $F[x]$ is a Euclidean domain, so it is a PID. This means that every ideal $I$ is generated by a single $f \in F[x]$. 
    \item $F[x]/f$ is a field if and only if $f$ is irreducible, meaning that it is equal to $f = gh$, where at least one between $g$ and $h$ is a polynomial of degree 0. 
    \item We can use CRT over $F[x]$, meaning that we can solve systems of congruence modulo pairwise coprime polynomials. 
    \item $(f) + (g) = (\gcd(f, g)) ; (f) \cap (g) = (\lcm(f, g))$. In these expressions, $\gcd$ and $\lcm$ are defined up to a unit, but there always exists a single monic solution. 
  \end{enumerate}
}
...
\aparte{Theorems}{
  ...
  \end{theorem*}
  \begin{proof}
    $f = gh, \deg(f) = \deg(g) + \deg(h) = 1 \Rightarrow \deg(g) = 1, \deg(h) = 0$ or vice versa. At least one of $h, g$ is a nonzero constant, meaning it is a unit. 
  \end{proof}
  \begin{theorem*}
    A polynomial of degree 2 or 3 is irreducible only if it has no roots in $F$. 
  \end{theorem*}
  \begin{proof}
    $f = gh, \deg f = \deg g + \deg h = 2 \text{ or } 3$. Therefore, in order for $f$ to be irreducible, then it has to be the case that at least one of $g, h$ is of degree 1, therefore being such that:
    \[
      g(x) = ax + b \stackrel{a \neq 0}= a\left(x + \frac{b}{a}\right) \Rightarrow g\left(-\frac{b}{a}\right) = f\left(-\frac{b}{a}\right) = 0
    \]
  \end{proof}
  \begin{theorem*}
    Let $f(x) = \sum_n a_ix^i$ of degree $n + 1$, with integer coefficients, considered over the rationals. Let $\alpha = \frac{r}{s}$ a root of $f$. Then, $s | a_n, r | a_0$. 
  \end{theorem*}
  \begin{proof}
    To convince yourself of this, consider: 
    \[
      f\left(\frac{r}{s}\right) \cdot s^n = 
      \rlap{$\overbrace{\phantom{a_nr^n + a_{n - 1}r^{n - 1}s + ... + a_1rs^{n - 1}}}^{r|}$} a_nr^n + \underbrace{a_{n - 1}r^{n - 1}s + ... + a_1rs^{n - 1} + a_0s^n}_{s|}
       = 0
    \]
    This means that $r | a_0$ and $s | a_n$. 
  \end{proof}
  \begin{theorem*}[Eisenstein criterion]
    Consider $f(x) = \sum_n a_ix^i$ with integer coprime coefficients ($\gcd(a_0, ..., a_n) = 1$). Suppose that $p$ is a prime such that $p | a_i, 0 \leqslant i \leqslant n - 1, p \not| a_n, p^2 \not|$. Then, $f$ is irreducible in $\mathbb{Q}[x]$. 
  \end{theorem*}
}
...
\aparte{Examples}{
  Let us consider some examples: 
  \begin{enumerate}
    \item Let $g(x) = 2x^3 + 4x^2 + 11x + 1$, over $\mathbb{Q}[x]$. If $r/s$ is a root, then $r|1$, and $s|2$. Therefore, $r \in \{\pm 1\}, s \in \{\pm 1, \pm 2\}$. This means that 
    \[
    ...
    \item $g(x) = x^k - p \in \mathbb{Q}[x]$ is always irreducible by Eisenstein. However, that is not the case for $x^{2k} - p^{2}$, since $p^2 | p^2$. 
  \end{enumerate}
}

\subsection{Quotient polynomial rings}
\begin{theorem*}
  If $f \in F[x]$ s irreducible, and of degree n, then any element $F[x]/(f)$ is of the form $\sum_{n - 1} a_i\bar{x}^i$, with $a_i \in F, \bar{x}^i = \{x^i + f(x)g(x)\}_{g \in F[x]}$. 
\end{theorem*}
...
\begin{theorem*}
  If $F$ is a finite field such that $|F| = q, f$ is an irreducible polynomial of degree $n$ in $F[x]$, then $K = F[x]/f$ has $q^n$ elements. 
\end{theorem*}
\aparte{Examples}{
  Consider $\mathbb{F}_2 = \mathbb{Z}/2\mathbb{Z}$, and consider $f(x) = x^3 + x^2 + 1 \in \mathbb{F}_2[x]$ of degree 3. Then $f(0) = 1 = f(1) \Rightarrow f$ has no roots in the field, meaning it has to be irreducible. Consider now $K = \mathbb{F}_2[x]/(f)$. Then, this is a field, of size $|K| = 2^3 = 8$, where every element is of the form $a\bar x^2 + b \bar x + c, a, b, c \in \mathbb{F}_2$. Additionally, we have that 
  \[
    \gcd(x, x^3 + x^2 + 1) = 1 \Rightarrow \exists\ h, g: xg(x) + (x^3 + x^2 + 1)h(x) = 1
  \]
  We find that $x {\color{redish}(x^2 + x)} + (x^3 + x^2 + 1) = 1$ in $\mathbb{F}_2$, meaning that the inverse of $\bar x$ is $(\bar x)^{-1} = (\bar x^2 + \bar x)$. 

  Another example: let $F = \mathbb{R}, f(x) = x^2 + 1$. It is irreducible since there are no roots in $\mathbb{R}$. Therefore, $\mathbb{R}[x]/(x^2 + 1)$ is a field, vector space of dimension 2 over $\mathbb{R}$ of the form $\{a + b\bar x, a, b \in \mathbb{R}\}$ and such that $(\bar x)^2 = -1$. This means that $\mathbb{R}/(x^2 + 1) \simeq \mathbb{C}$. 
}
...
\aparte{Conclusions}{
  ...
  \begin{enumerate}
    \item For any prime $p$ and $n \geqslant 1$, there exists a unique field $\mathbb{F}_{p^n}$ of $p*n$. 
    \item $n = 1 \Rightarrow$ this unique field is isomorphic to $\mathbb{F}_{p} \simeq \mathbb{Z}/p\mathbb{Z}$. 
    \item $n > 1 \Rightarrow$ we can construct it as a quotient $\mathbb{F}_{p^n}/(f)$, with $f$ an $n$-degree polynomial over $\mathbb{F}_{p}$. 
  \end{enumerate}
}
\begin{corollary}
